FindClick(ImageFile="", Options="", ByRef FoundX="", ByRef FoundY="") ; updated January 28 2013 ... http://www.autohotkey.com/board/topic/89592-findclick-2nd-edition
{
	Static Cache
	, Center, Silent, Delim, Count, CharX, CharY, CharN, Sleep, Stay, dx, d, m, x, y, o, n, k, r, e, a, w, t, f
	...
	, GuiHWND, GuiCommand, GuiTitle, # := "`t", $ := "`r"
	...
	Literal = "
	UseRGB = %False%
	ForumURL = http://www.autohotkey.com/board/topic/89592-findclick-2nd-edition
	DocumentationURL = https://www.cubby.com/p/_a0539a2786cd4f87affe8f74efd05be7/FindClick.pdf
	;||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||
	If (ImageFile <> "") and (Asc(ImageFile) <> 62) {
		If (LastImageFile <> ImageFile) or (Options <> LastOptions) {
			...
			If (SpecialLaunch <> 3) {
				...
				;----------------------------------------------------------------------------------------------------
				UserConfig_A = dx"SaveTo=%A_Desktop%\FindClickDebug.txt"
				;||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||
			}
			@5 := 1, TempOptions := Options, Commands := "Center|Silent|Delim|Count|CharX|CharY|CharN|Sleep|Stay|Err|dx|d|m|x|y|o|n|k|r|e|a|w|t|f"
			If SpecialLaunch and (SpecialLaunch <> 4) {
				...
				Description     The given string represents a template into which image x, y, and item numbers will be substituted. Each time an image is found, the CharX, CharY, and CharN characters will be replaced for these values. The final string is returned by the function.
				--------------------------------------------------------------------------------
				...
				--------------------------------------------------------------------------------
				Silent          No Dialogs
				Description     Instead of displaying an error dialog when an error prevents the function from executing properly (for instance, if the image file is not found), it will silently return a blank string and set the errorlevel to the error message. The errors concerned (which are numbered 1-7 in the code) should not appear during normal execution and so Silent is generally not recommended.
				--------------------------------------------------------------------------------
			...
			While (@5 := RegExMatch(TempOptions, "i)(?:^|\s)(?:!(\w+)|(\+|-)?(" Commands ")(" Literal "(?:[^" Literal "]|" Literal Literal ")*" Literal "(?= |$)|[^ ]*))", @, @5 + StrLen(@))) {
				If (@1 <> "") {
					TempOptions := SubStr(TempOptions, 1, @5 + StrLen(@)) UserConfig_%@1% SubStr(TempOptions, @5 + StrLen(@))
					If DxInfo
			...
			If DxInfo
				...
				. "q:" DllCall("QueryPerformanceCounter", "Int64*", QPC) * QPC #
				. "t:Certain information has to be retrieved from the given image file." #
				. "l:" A_LineNumber + 1
				...
				If RegExMatch(ImageFile, "i)^\*(?:0x)?([0-9A-F]{2})([0-9A-F]{2})([0-9A-F]{2})(?:\D*(\d+)\D*(\d+).*?)?$", @) {
					...
					Loop, Parse, DefaultDirs, |
					{
						...
						If DxInfo
							...
							. "q:" DllCall("QueryPerformanceCounter", "Int64*", QPC) * QPC #
							. "t:Directory #" A_Index " is: %ThisDir%`n`n(This can be specified at the top of the script in DefaultDirs.)" #
						...
						Loop, Parse, DefaultExts, |
							If FileExist(ThisDir ImageFile A_LoopField) {
								...
								. "q:" DllCall("QueryPerformanceCounter", "Int64*", QPC) * QPC #
								. "t:Trying: %ImageFilePath%`nFile does not exist." #
					...
					If (ImageFilePath = "") {
						If !Silent {
							...
							Message = Image file "%ImageFile%" not found.`nCreate it now?
							Buttons = &Yes,&No,&Debug,&Help
							Result := %A_ThisFunc%(">Error<", Error "`n" Message "`n" Buttons)
							If Result = Yes
						...
						. "q:" DllCall("QueryPerformanceCounter", "Int64*", QPC) * QPC #
						. "t:Trying: %ImageFilePath%`nImage file found." #
					...
					If !%A_ThisFunc%(">ParseCache<", ImageFilePath) {
						...
						Gui, %GuiA%:Destroy
						If (ImageW = "") {
							If (%A_ThisFunc%(">Error<", "Image Problem`rImage file """ ImageFile """ appears to be of an unsupported filetype.`n`nDebug now?") = "Yes")
								Return %A_ThisFunc%(ImageFile, Options " dx")
							Return
						}
						If DxInfo
		...
		If (r <> "") {
			...
			If (Search1 = "") or (Search3 <= 0) or (Search4 <= 0) {
				...
				If DxInfo
					DxInfo .= $ "2:Window not found" #
					. "q:" DllCall("QueryPerformanceCounter", "Int64*", QPC) * QPC #
					. "t:yyyy"
				ErrorMessage = Window Not Found
				ErrorText = Window "%r%" was not found.
				ErrorHelpUrl = #r
				return ;xxx
				Return %A_ThisFunc%(">Error<", ErrorMessage "`r" ErrorText "`r" ErrorHelpUrl)
			} Else If DxInfo
		...
		Loop {
			...
			If (d = "") or (SubRegions <> "`n") {
				If DxInfo
					...
					. "q:" DllCall("QueryPerformanceCounter", "Int64*", QPC) * QPC #
					. "t:The following line of code will execute " Search5 ":`n`n%Code_Orig%" #
					. "l:" A_LineNumber + 2 #
			...
			If (ErrorLevel = 2) {
				...
				If NewImage
					Return %A_ThisFunc%(ImageFile, Options)
				ErrorLevel := "[7]: AutoHotkey was unable to use the image """ ImageFile """ for searching."
				If !Silent
					MsgBox, 262160, %A_ScriptName% - %A_ThisFunc%(): Error, %ErrorLevel%
				Return
			} Else If !ErrorLevel {
				If DxInfo
					DxInfo .= $ "2:" (SubRegions = "`n" ? "Selecting " d "-most Image" : "Search #" A_Index " - Success") @
					. "`r3:Image Found at (" FoundXOrig ", " FoundYOrig ")" #
					. "q:" DllCall("QueryPerformanceCounter", "Int64*", QPC) * QPC #
					. "t:yyyy" #
					. "l:" A_LineNumber - 3 #
					. "d:5" #
					...
					If n {
						...
						If (InStr(m, "c") = 1) {
							...
							Loop %WinList% {
								...
								If (FoundX >= @1) and (FoundY >= @2) and (FoundX <= @1 + @3) and (FoundY <= @2 + @4) {
									...
									ErrorLevel := "[6]: " A_ThisFunc "() was unable to locate a window to use for a ControlClick keystroke."
									If !Silent
										MsgBox, 262160, %A_ScriptName% - %A_ThisFunc%(): Error, %ErrorLevel%
									Return
								}
						...
						Loop % n * 2 - 1 {
							If DxInfo
								...
								. "t:The following line of code will execute:`n`n" (!(A_Index & 1) ? "Sleep, " Sleep : SubStr(m, 1, 1) = "i" ? "SendInput, " k : SubStr(m, 1, 1) = "c" ? "ControlClick, x" FoundX - ClickX " y" FoundY - ClickY ", ahk_id " Window ", , " k ", , NA" : SubStr(m, 1, 1) = "p" ? "SendPlay, " k : SubStr(m, 1, 1) = "e" ? "SendEvent, " k : "Send, " k) #
					...
					If d
						If (Asc(d) = 108) { ; l (Left)
							...
							If (Dist1 = "") {
								...
								Else
									StringSplit, Dist, d, |`,
;~ 								MsgBox, 262144, %A_ScriptName%: Alert, % dist1 A_Space dist2
				...
				If DxInfo
					...
					. $ "3:Image Not Found" #
					. "t:The image was not found in the given region.`n`n" (Results ? (SubRegions = "" ? "No more regions remain to be searched (""e"" option) so the script will now finish." : "Moving on to the next region:`n" SubStr(SubRegions, 1, InStr(SubRegions, "`n") - 1)) : w = "" ? "Because the user did not instruct the function to continue searching until the image was found (""w"" option), the script will now finish." : w1 ? "The ""w"" option was indicated as " w " and " A_ThisFunc "() has continued to search for the image. However the wait time has now expired and the script will finish.") #
...
;~ 					Break
				...
				If DxInfo
					...
					. "`r3:Sleeping" #
					. "q:" DllCall("QueryPerformanceCounter", "Int64*", QPC) * QPC #
					. "t:The ""w"" option (wait for an image to be found) was given as: " w ". Since no image has been found yet, the function now sleeps " w2 " milliseconds." #
					. "l:" A_LineNumber + 1
				Sleep w2
	...
	} Else If (InStr(ImageFile, ">Error<") = 1) { ;--------------------------------------------------Displays & handles error dialogs--------------------------------------------------
		If DxInfo
			DxInfo .= $ "1:Finishing" #
				. "q:" DllCall("QueryPerformanceCounter", "Int64*", QPC) * QPC #
			...
			, %A_ThisFunc%(">Debug:", dx)
		Else If !Silent {
			Error := SubStr(Options, 1, InStr(Options, "`n") - 1), Buttons := SubStr(Options, InStr(Options, "`n", 0, 0) + 1), Message := SubStr(Options, StrLen(Error) + 2, -StrLen(Buttons) - 1)
			Margins = 10
			MsgBoxWidth = 250
			ButtonDebug = &Debug
			ButtonNo = &No
			...
			Loop, Parse, Buttons, CSV
				ButtonList .= ";Add,Button,g" A_ThisFunc "Close y" (A_Index = 1 ? "+" Margins " x" : "p x+") Margins " w" (MsgBoxWidth - Margins * 2) // 3 " h" ControlHeight "," A_LoopField
			WinWaitClose, % "ahk_id " %A_ThisFunc%(">NewGui<", GuiB . ";+AlwaysOnTop +Label" A_ThisFunc . ";Add,Picture,Icon132 x" Margins " y" Margins ",Shell32.dll" . ";Add,Text,x+" Margins " y" Margins " w" MsgBoxWidth - Margins - 32 "," Message . ButtonList . ";Show,w" MsgBoxWidth + Margins * 2 ",Error: " Error)
			If (GuiCommand = ButtonHelp)
				Run, %Message3%
			Else If (GuiCommand = ButtonDebug)
				@ := LastImageFile, LastImageFile := ""
				, %A_ThisFunc%(@, LastOptions " dx")
		}
		Return RegExReplace(GuiCommand, "(?<!&)&(?!&)"), ErrorLevel := Error
		...
		MenuSettingsShowChars = Easy &String Display
		MenuSettingsWrapText = &Wrap Text
		MenuSettingsEdit = &Edit Debugger Settings
		MenuSettingsSkipSteps = Skip Unimportant Steps ; xxx
		...
		MenuStandard := True, Hotkeys := "Play,Next,Prev,CollapseExpand", Buttons := "Button1,Button2,Button3,Edit2"
		...
		. ";Add,Button,g" A_ThisFunc "Close x" Margins " y" Margins " w" (Width - Margins * 3) / 5 " h" ControlHeight "," ButtonPrev
		...
		. ";Add,Edit,-VScroll +ReadOnly x" Margins * 4 + (Width - Margins * 3) * 3 / 5 " y" Margins " w" (Width - Margins * 3) * 2 / 5 " h" ControlHeight
		. ";Default")
		...
		Loop, Parse, DxInfo, `r, `n
			If (A_Index = 1) {
				...
				Loop, Parse, A_LoopField, %A_Tab%
					If (A_Index = 1) {
						%Items%_Level := @1 := SubStr(A_LoopField, 1, 1), @2 := @1 - 1, %Items%_ID := TV%@1% := TV_Add(%Items%_Title := SubStr(A_LoopField, 3), TV%@2%)
		...
		Loop {
			...
			If (TempGuiCommand = ButtonPlay) or (TempGuiCommand = ButtonPause) or (TempGuiCommand = HotkeyPlay) or (Playing and (TempGuiCommand = "ErrorLevel")) {
				Playing := !Playing, %CurrentStep%_d := Floor(%CurrentStep%_d)
			...
			} Else If (TempGuiCommand = MenuSettingsCompactView) or (TempGuiCommand = HotkeyCollapseExpand) {
				If (TempGuiCommand = HotkeyCollapseExpand)
					Menu, %A_ThisFunc%MenuSettings, % (CompactView := !CompactView) ? "Check" : "Uncheck", %MenuSettingsCompactView%%A_Tab%%HotkeyCollapseExpand%
				Loop % n := 10 {
			...
			Else If (TempGuiCommand = MenuFileNext) or (TempGuiCommand = ButtonNext) or (TempGuiCommand = HotkeyNext) or ((TempGuiCommand = "") and Playing) {
				Loop
					If (CurrentStep = Items) {
						...
						CurrentStep += 1
						If (TempGuiCommand = ButtonNext) or (%CurrentStep%_d <> "")
							Break
					}
				If (CurrentStep = Items)
					Continue
			...
			If (GuiCommand = "") {
				...
				WinWaitClose, %GuiTitle% ahk_id %GuiHWND%, , Playing ? %CurrentStep%_d * PlaySpeed : ""
			}
		}
		...
		InputBoxWidth = 3 00              ; Width of the dialog box used for adding new options
		...
		Loop, Parse, OptionInfo, `r
		{
			Loop, Parse, A_LoopField, `n
			{
				StringSplit, @, A_LoopField, %A_Tab%
				If (A_Index = 1)
					Commands .= "|" Item := @1, @1 := "Main" ; xxx still using %Commands% ?
				If (@2 <> "")
		...
		Commands := SubStr(Commands, 2), OptionInfo := GuiTitle := "", LastPos := "x-300 y-300 w10 h10"
		Loop {
			If (FilePaths = "") {
